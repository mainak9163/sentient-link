...
export function LoadingState({
  ...
  if (useCustomAlias) {
    return (
      <Alert
        className={`border-violet-200 bg-violet-50/50 dark:border-violet-900 dark:bg-violet-950/50 backdrop-blur-sm ${
          className ?? ""
        }`}
        {...props}
      >
  ...
  return (
    <Alert
      className={`border-cyan-200 bg-cyan-50/50 dark:border-cyan-900 dark:bg-cyan-950/50 backdrop-blur-sm ${
        className ?? ""
      }`}
      {...props}
    >
